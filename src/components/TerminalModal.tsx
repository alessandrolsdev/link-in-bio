...
/**
 * Modal de Terminal Interativo (AlessandroOS).
 * Simula um sistema operacional baseado em terminal.
 * Permite que o usuário interaja via comandos textuais para descobrir mais sobre o perfil.
 * 
 * Comandos Suportados: help, about, skills, contact, clear, exit.
 */
export const TerminalModal = () => {
  ...
  // Auto-scroll e Auto-focus ao abrir ou atualizar o histórico
  useEffect(() => {
  ...
  const handleCommand = (e: React.FormEvent) => {
    ...
    // Processamento de comandos especiais e respostas padrão
    if (cmd === "clear") {
      setHistory([]);
    } else if (cmd === "exit") {
      setIsOpen(false);
      ...
      {isOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
          <div className="w-full max-w-2xl bg-[#0c0c0c] border border-zinc-800 rounded-lg shadow-2xl overflow-hidden font-mono text-sm flex flex-col h-[400px]">
            {/* Barra de Título (Window Controls) */}
